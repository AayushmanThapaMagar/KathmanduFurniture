  ...
  return (
    <div className="flex justify-center">
      <div className="flex flex-row w-full lg:mx-10 h-28 bg-[#2596be] relative rounded-lg items-center lg:px-20 text-gray-100">
        {stats.map((stat) => (
          <div
          className="group flex flex-col lg:w-20 lg:h-20 w-10 h-20 text-center flex-grow gap-y-2 border-2 border-[#2596be]"
          >
            <div className="flex-shrink-0 place-self-center group-hover:scale-110 transition duration-500 ease-in-out">{stat.icon}</div>
            <div className="lg:text-xl lg:font-bold text-xs font-normal">{stat.title}</div>
          </div>
        ))}
      </div>
    </div>
